...
async def input_image_primpt(clbck: CallbackQuery, state: FSMContext):
    await state.set_state(Gen.img_prompt)
    await clbck.message.edit_text(text.gen_image)
    await clbck.message.answer(text.gen_exit, reply_markup=kb.exit_kb)

@router.callback_query(F.data == "help")
async def help_message(clbck: CallbackQuery, state: FSMContext):
    await clbck.message.edit_text(text.help)
    await clbck.message.answer(text.help_exit, reply_markup=kb.exit_kb)
